...
endDate <- as.Date("2012-02-28")
# the length of the period in unit of week
nWeek <- as.numeric(endDate - beginDate + 1)/7
...
full.link$Freq <- full.link$Freq + full.link$total
...
# MinPts is the minimum number of neigbors. Chosen as 3. 
...
plot(small.station$long, small.station$lat, 
  type="p", pch=19, 
  col=c("grey", color)[cluster1$cluster + 1])

############################### popularity analysis #############################
# histogram of pathIDs
freq.table <- hist(full.link$Freq/nWeek, breaks=seq(0,200, by=2))
...
ylab="10-based log of number of pathIDs", xlab="Average exchanges per week",
...
pop.path$Freq <- pop.path$Freq/nWeek				      # and 31104-31106

# popular stations
pop.start <- as.data.frame.table(table(jan05$Start.Term)/nWeek)
pop.start <- pop.start[order(pop.start$Var1),]
pop.end <- as.data.frame.table(table(jan05$End.Term)/nWeek)

pop.station <- pop.start
pop.station$Freq <- pop.start$Freq + pop.end$Freq[order(pop.end$Var1)]
