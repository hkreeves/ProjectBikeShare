...
# hand side of the dendrograma€¡±I cana€?t figure it out. If ita€?s not this, we should #
# output the modularity score and plot that [same for edge betweenness a€¡° what 
...
# walks of lengths 1a€¡°50) and selected the network partition with the highest 
# modularity value from those 50. They call this the a€?maximum modularity 
# partitiona€? and insert the parenthetical a€?(though, strictly speaking, this cannot be 
# proven to be the optimum without computationally-prohibitive exhaustive 
# enumeration (Brandes et al. 2008)).a€?


friend_comm_wt <- walktrap.community(m182_friend_no_iso, steps=200,modularity=TRUE) #,labels=TRUE)
friend_comm_wt
# [edit: Kai] Weighted by attribute "friend_tie"
friend_comm_wt2 <- walktrap.community(m182_friend, weights=E(m182_friend)$friend_tie, steps=200,modularity=TRUE) #,labels=TRUE)
friend_comm_wt2
...
#friend_comm_dend <- as.dendrogram(friend_comm_wt2, use.modularity=F)
...
# is likely that edges connecting separate modules have high edge betweenness as 
# all the shortest paths from one module to another must traverse through them. 
...
for (i in 0:(num_vertices-1)) {
	...
	if (num_comms > 1 & num_comms < num_vertices) {
		ideal_observed_cors <- append(ideal_observed_cors, (gcor(idealized_comm_mat, as.matrix(friend_adj_mat_no_iso))))
...
# IN ORDER TO PERFORM THE COMMUNITY DETECTION ALGORITHMS, WE HAD TO REMOVE THE ISOLATES FROM THE GRAPH 
# (HENCE THE NO_ISO MATRICES ABOVE). NOW WE WANT TO RE-INSERT THEM, SO WE ASSIGNING EACH TO ITS OWN 
# "COMMUNITY" (NUMBERED 3 AND 4) AND MANUALLY ADD THESE COMMUNITIES TO THE VECTOR OF COMMUNITIES IN 
